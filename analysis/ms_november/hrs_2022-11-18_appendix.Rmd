---
title: Appendix
subtitle: Detailed description of parameter creation
author:
...
This appendix details the calculations used to determine the available forest
residue biomass available on each township in each year of the study period.

# Utilization standards

For administrative purposes, the Green Area of Alberta is divided into Forest
...
comprises 12 FMUs. Maps of the FMUs @AlbertaFMU and the FMAAs @AlbertaFMA can be
...
Ref. @NRCan2015 presents taper models and the associated coefficients for
...
Ung *et al.* @Ung2013 describe a one-parameter model which is useful when both
...
Ung *et al.* @Ung2013 also present a simple linear model relating outside bark to
...
approximately normal @Bettinger2017. We use this to justify the use of the
average tree.
...
determined for each  township in Alberta.
...
Canada @Beaudoin2014 to retrieve information on species composition, age
...
Canada's National Forest Inventory (NFI henceforth).
...
corresponds with stand merchantability limits specified in Alberta. We
...
@CCFM2020. The average township volume after the filter was applied was
...
For each township, we calculate the number of stems by dividing the adjusted 
volume per hectare by the average piece size for both needleleaf and broadleaf
trees. We assume that for
the types of stands being harvested that the stem count per hectre for the 13/+7
utilization standard is close to the stem count based on the FMU utilization
standard. We multiply the stem count by the average tree residual biomass as 
calculated in section 2.4.
We assume an average recovery factor of 0.624 following what Peltola et al.
@Peltola2011 determined for Finland, recognizing that some of the residual 
biomass will be unrecoverable. Together this gives us an estimate of the average
residue yield when a hectare of a stand in a particular township is harvested.
