...
```{r BRIMS, echo=FALSE, fig.cap="BRIMS residue", out.width = '100%'}
knitr::include_graphics("../img/BRIMS.png")
```
...
other_cost <- 8.48 # $/odt
constant_cost <- prepiling_cost +  comminution_cost + other_cost # $·Mg^-1^ (DM)
...
different plant capacities: 200 000 Mg/a (DM) corresponding to a small wood pellet
mill, 400 000 Mg/a (DM) corresponding to a large pellet mill or a small ethanol
plant, and 800 000 Mg/a (DM) corresponding to a large ethanol plant.

```{r bb, echo=FALSE, fig.cap="Boreal Bioenergy--Obed.", out.width = '80%'}
knitr::include_graphics("../img/BorealBioenergy.png")
```
...
## Residue yield (Mg · ha^-1^ (DM)) by township.
...
\[
a(d_b,h) = \pi \left(\frac{d(d_b,h)}{200}\right)^2
\]

\[
v(d_b,h_s,h_t) = \int_{h_s}^{h_t}\pi \left(\frac{d(d_b,h)}{200}\right)^2dh
\]

The constant 200 is used to convert a diameter (cm) to a radius (m).  
**find a way to reduce possible confusion with $d$**
...
For both needleleaf and broadleaf, we multiply the residue associated with the
individual tree by the number of harvested stems per ha.  We reduce this
value using a recovery factor of 0.645 **??** following Peltola *et al.* 
...
```{r roser, echo=FALSE, fig.cap="Röser.", out.width = '100%'}
knitr::include_graphics("../img/roser.png")
```

  
```{r hourly_rates, echo=FALSE, fig.cap="Hourly rates for forestry equipment.", out.width = '100%'}
knitr::include_graphics("../img/hourly_rates.png")
```
  


Following @Roser2013, the pre-piling cost was set to
`r sprintf("%5.2f",prepiling_cost)` \$·Mg^-1^ (DM), the comminution (*grinding ?*) cost
was set to `r sprintf("%5.2f",comminution_cost)`  \$·Mg^-1^ (DM), and other costs were
set to  `r sprintf("%5.2f",other_cost)` \$·Mg^-1^ (DM).  The total "constant" costs are
`r sprintf("%5.2f",constant_cost)` \$·Mg^-1^ (DM).
Using *blah blah blah* , the truck rate was set to 
`r sprintf("%5.2f",truck_rate)` \$·Mg^-1^ (DM)/h.
...
p <- ggplot(t,aes(factor(year),residueCost))  + 
  ...
  labs(y = "residue cost ($·Mg^-1^ (DM))", x="year") +
...
also affects Hinton and Edson at 800 000 Mg/a (DM) capacity, 
