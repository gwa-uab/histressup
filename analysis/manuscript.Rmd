...
Need to reference Yemshanov @Yemshanov2014 and Blackburn @Blackburn2020.  Our
strengths
...
used in our analysis.  Most of the data are freely available from the World
...
```{r canada, echo=FALSE, fig.cap="Location of Alberta in Canada.", out.width = '80%'}
...
different plant capacities: 200 000 Mg/a  corresponding to a small wood pellet
mill, 400 000 Mg/a  corresponding to a large pellet mill or a small ethanol
plant, and 800 000 Mg/a  corresponding to a large ethanol plant.
...
generated using the QGIS plugin, QNEAT3
(Iso-Area as Interpolation) @Raffler2018.
QGIS zonal statistics were used to calculate the median network travel time to
each township, from each of our three towns.  The one-way travel time in Figure
...
`r sprintf("%5.2f",prepiling_cost)` \$·Mg^-1^ , the comminution cost
...
@TT2020.  The rate reported for an 8 axle tridem tractor with a
b-train trailer was `r sprintf("%5.2f",hourly_truck_rate)` \$/h.  We assume that
...
harv_comparison <- ggplot(data=harv5, aes(x=year,y=harvest_vol, colour="Estimated harvest")) +
  labs(x = "year") +
  labs(y = bquote('harvest volume' ~(10^6 ~m^3))) +
  geom_line()  +
  geom_line(aes(y=total, colour="Actual harvest"))  +
  scale_y_continuous(limits=c(0,30e6),labels = label_number(scale = 1/1e6)) +
...
```{r resdiue, echo=FALSE, fig.cap="Biomass residue available from harvest.", out.width = '100%'}
knitr::include_graphics("../img/residue_per_ha.png")
```

## Distributional results
```{r supply_curves, echo=FALSE, warning=FALSE, message=FALSE, fig.cap="Residue supply (cycle time) curves by location.  Each curve represents one year.  The red curve is for 2009, which was the year of median harvest volume."}

cyclePlot <- ggplot(data=cum.odmt, aes(x=residue.cumsum,y=cycleTime,group=year,colour=as.factor(year))) +
  labs(x = "cumulative residue (1000 Mg )") +
  labs(y = "marginal cycle time (hr)") +
  geom_line() +
...
```{r more_supply_curves, echo=FALSE, warning=FALSE, message=FALSE, fig.cap="Residue supply (delivered cost) curves by location.  Each curve represents one year.  The red curve is for 2009, which was the year of median harvest volume."}
mcPlot <- ggplot(data=cum.odmt, aes(x=residue.cumsum,y=residueCost,group=year,colour=as.factor(year))) +
  labs(x = "cumulative residue (1000 Mg )") +
  labs(y = "marginal cost ($·Mg^-1^ )") +
  geom_line() +
  scale_colour_manual(values=c("Grey","Grey","Grey","Grey","Grey","Grey","Grey",
                               "Grey","Grey","Grey","Grey","Grey","Grey","Grey",
                               "Grey","Grey","Grey","Grey","Grey","Red","Grey",
...
bPlot <- ggplot(t,aes(factor(year),residueCost))  + 
  ...
  labs(y = "residue cost ($·Mg^-1^ )", x="year") +
...
16 years of the study period.  The year 2009 had the median overall harvest 
level in Alberta over that period, and is coloured red in order to illustrate
the marginal cycle time for a median year.



```{r cyclePlot, echo=FALSE, fig.cap="Distribution of cycle times by location.", out.width = '100%'}
suppressWarnings(print(cyclePlot))
```


```{r mcpPlot, echo=FALSE, fig.cap="Distribution of marginal costs by location.", out.width = '100%'}
suppressWarnings(print(mcPlot))
```

```{r bPlot, echo=FALSE, fig.cap="Boxplots of costs by location.", out.width = '100%'}
...
also affects Hinton and Edson at 800 000 Mg/a  capacity, 
...
```{r w2009, echo=FALSE, fig.cap="Relationship between timber harvest intensity and required supply area for facilities with demands of 200, 400, and 800 Tg.  The area of the dots reflects the proportion of the area of each township harvest in 2009", out.width = '100%'}
knitr::include_graphics("../img/w2009.png")
```


```{r wcontour, echo=FALSE, fig.cap="Supply area for facilities in Whitecourt with demands of 200, 400, and 800 Tg for low cost (2005), medium cost (2009), and high cost (1990) years.", out.width = '100%'}
knitr::include_graphics("../img/wcontours.png")
```
# Conclusions
