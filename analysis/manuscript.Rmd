...
different plant capacities: 200,000 odmt/yr corresponding to a small wood pellet
mill, 400,000 odmt/yr corresponding to a large pellet mill or a small ethanol
plant, and 800,000 odmt/yr corresponding to a large ethanol plant.
...
The NFIS' individual tree biomass calculator @NFIS2016 uses province, ecozone,species, dbh, and height as input variables, so ecozone is important.  It is based on 
...
```{r biomass calculator example, echo=FALSE, fig.cap="Biomass calculator example.", out.width = '80%'}
...
(*Larix spp.*) and birch (*Betula spp.*) as those genera are rarely harvested at
...
are white spruce (_Picea glauca_), lodgepole pine (*Pinus contorta* var.
*latifolia*), trembling aspen (*Populus tremuloides*), and less commonly jack
pine (*Pinus banksiana*), black spruce (*Picea mariana*), balsam poplar
(*Populus balsamifera*), subalpine fir (*Abies lasiocarpa*), balsam fir (*Abies
balsamea*), and Douglas-fir (*Pseudotsuga menziesii* var. *glauca*).
...
Following @Roser2013, the pre-piling cost was set to `r
sprintf("%5.2f",prepiling_cost)` \$/odmt, the comminution (*grinding ?*) cost
was set to `r sprintf("%5.2f",comminution_cost)`  \$/odmt, and other costs were
set to  `r sprintf("%5.2f",other_cost)` \$/odmt.  The total "constant" costs are
`r sprintf("%5.2f",constant_cost)` \$/odmt.
Using *blah blah blah* , the truck rate was set to `r
sprintf("%5.2f",truck_rate)` \$/odmt/h.
